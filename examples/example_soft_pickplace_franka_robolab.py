"""``soft_pickplace_franka`` with RoboLab's graphics instead of the Newton viewer.

The physics is *identical* to ``examples.soft_pickplace_franka`` — this module subclasses its
``Example`` and only swaps the visualization layer for the RoboLab (NVIDIA DROID rig) look
reproduced by ``robolab_viz``. The Franka picks a small graspable soft FEM block off the table
and places it at a target location; the dynamic finger proxies grip the deformable block (the
squeeze + soft contact read clearly in the flat-shaded preview).

- the scene from RoboLab's ``run_recorded.py``: home-office dome background, sphere key light,
  maple work table, Franka pedestal, robot base at the viz-frame origin;
- both policy camera perspectives: the over-shoulder-left exterior camera and the
  gripper-mounted wrist camera;
- the robot rendered from an Isaac-converted USD of the *same* URDF that is simulated,
  puppeteered per frame from the solver state.

Outputs per run (in ``outputs/robolab_preview/``):

- ``combined.mp4`` — the warp-raycast preview video (the configured cameras side-by-side,
  flat-shaded but geometrically exact), produced on any CUDA GPU. This is the video kept.
...
- ``--usd`` writes ``outputs/soft_pickplace_franka_robolab.usd`` — the full RoboLab-styled
  scene with one time sample per frame (open in usdview/Omniverse or RTX-render with
  ``python -m robolab_viz.isaac_render`` on an RTX-capable GPU).
- ``--npz`` writes the per-frame state cache (+ ``robolab_preview/geometry.pkl``) that
  ``python -m robolab_viz.rerender`` replays to re-render cameras without re-simulating.

Run:

    python -m examples soft_pickplace_franka_robolab --viewer null --device cuda:0
"""
...
import examples
import newton.utils
from examples.soft_pickplace_franka import Example as SoftPickPlacePhysicsExample
...
class Example(SoftPickPlacePhysicsExample):
    def __init__(self, viewer, args):
        if args.viewer != "null":
            raise SystemExit(
                "soft_pickplace_franka_robolab renders through robolab_viz (USD + raycast preview); "
                "run it with --viewer null."
            )
        super().__init__(viewer, args)

        # Mirror the base example's robot import (same URDF asset, same base pose as
        # build_franka_robot). The FK/sim parity assertion in test_final() guards these.
        ...
        # RoboLab's DROID scene, with the visual work-table placed from THIS demo's physics table
        # so the rendered surface coincides with the invisible contact table.
        ...
        # Optional fixed object-view camera framed on the soft block's pick location.
    ...
    def test_final(self) -> None:
        ...
        # Viz/sim parity: the FK mirror must agree with the simulated robot on the shared bodies.
        link_tfs = self.viz_fk.link_world_transforms(self.robot_state_0.joint_q)
        ...
        # The visible work table must cover the physics contact footprint and align its top.
    ...
    @staticmethod
    def create_parser():
        parser = SoftPickPlacePhysicsExample.create_parser()
        parser.set_defaults(output_path=str(Path("outputs") / "soft_pickplace_franka_robolab.usd"), viewer="null")
        ...
        parser.add_argument("--objectview", type=_str2bool, nargs="?", const=True, default=False,
                            help="Add a fixed object-inspection camera framed on the soft block; its PNG "
...
if __name__ == "__main__":
    ...
    viewer, args = examples.init(parser, example_name="soft_pickplace_franka_robolab")
