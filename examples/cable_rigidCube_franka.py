"""Franka grasps a cable off the table, lifts it, and sweeps it side to side past a rigid cube.
8 substeps. Pure SCENE + POLICY; all physics/robot/asset detail comes from
:mod:`deformableManipulationTools`, all rendering from :class:`robolabViz.scenic.ScenicGraspExample`.

Run: python -m examples cable_rigidCube_franka --device cuda:0
  (scenic by default -> outputs/cable_rigidCube_franka/{frames/, simulation.mp4};
   --output-style basic writes outputs/cable_rigidCube_franka.usd instead.)
"""
...
class Example(ScenicGraspExample):
    def configure(self, args):
        ...
        # Force-stop grasp on the cable (incompressible → latch at FIRST contact; no preset width).
        # The cable is a loose LINE-contact cage, so it over-closes by CABLE.grip_bite (more than the
        # default 1 mm — see params) for a firm cage that survives the sweep. Hold to the end of the run.
        self.grasp_windows = [GraspWindow(close_start=2.8, close_end=4.6, grip_bite=CABLE.grip_bite)]
    ...
    def _cable_layout(self, start_pos):
        ...
        return [
            (start_pos + direction * CABLE.segment_length * i
    ...
    def plan(self, ik_model, ik_state):
        ...
        self.cable_node_positions = self._cable_layout(start)
