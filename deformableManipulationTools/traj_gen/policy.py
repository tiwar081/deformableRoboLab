...
def resolve_place(scene: dict, task: dict, placement: dict, catalog_by_name: dict,
                  target_name: str, target_xy) -> dict:
...
class TrajPlan:
    ...
    def to_dict(self) -> dict:
        return {"version": 1, "target": self.target, "target_label": self.target_label,
                "target_ordinal": self.target_ordinal, "attempt": self.attempt,
                "pick": self.pick, "place": self.place, "phases": self.phases,
                "grasp_window": self.grasp_window, "num_frames": self.num_frames,
...
def choose_target_index(scene: dict, task: dict, catalog_by_name: dict) -> int:
    ...
    ref_half = _xy_half(_entry_dims(catalog_by_name, str(ref_name)))
    for i in t_idx:
        o = scene["objects"][i]
...
def plan_pick_place(scene: dict, task: dict, placement: dict, catalog_by_name: dict,
                    ...
                    target_index: int | None = None) -> TrajPlan:
    """Assemble the full timed plan for one selected grasp.
    ...
    target = task["goal"]["params"]["object"]
    t_idx = object_indices(scene, target)
    if not t_idx:
        raise PlanError(f"task object {target!r} is not in the scene")
    chosen = target_index if target_index is not None \
        else choose_target_index(scene, task, catalog_by_name)
    tgt = scene["objects"][chosen]
    target_xy = (float(tgt["x"]), float(tgt["y"]))
    place = resolve_place(scene, task, placement, catalog_by_name, target, target_xy)

    from agentic_pipeline import geometry  # lazy (same reason as resolve_place)
    ...
    settled_z = float(tgt.get("z", TABLE.top_z - float(target_bottom_dz)))
    obj_bottom_z = settled_z + float(target_bottom_dz)
    drop = max(float(gpos[2] - obj_bottom_z), 0.0)
    ...
    # -- cruise: parked -> above the pre-grasp (empty hand). The routed Bezier runs between
    # ELEVATED endpoints (each raised to its own column's clear height over the INFLATED boxes);
    # the final drop to the pre-grasp is a straight vertical connector — deliberately outside the
    # routed field, because the inflation is conservative box-swelling while the low approach is
    # exactly what grasp_select's corridor clearance validated against the TRUE boxes. --
    ...
    leg = curve.route(parked, approach_top, cruise_field)
    routing["legs"].append({"name": "cruise", "inserted": leg.inserted, "clear": leg.clear,
                            "blockers": list(leg.blockers)})
    if not leg.clear:
        raise PlanError(f"cruise leg cannot be routed clear (blockers: {leg.blockers})")
    t = SETTLE_T
    for tt, p, via in curve.leg_waypoints(leg, t, CRUISE_SPEED):
        wps.append(_wp(tt, p, via=via, **ori))
        t = tt
    phases.append({"name": "cruise", "t0": SETTLE_T, "t1": t})

    # -- descend: straight down to the pre-grasp, then the validated corridor to the grasp --
    ...
    phases.append({"name": "descend", "t0": t, "t1": t_desc})

    # -- close + hold --
    close_start = t_desc + PRE_CLOSE_DWELL
    close_end = close_start + CLOSE_DUR
    t_hold = close_end + POST_CLOSE_DWELL
    wps.append(_wp(t_hold, gpos, **ori))
    phases.append({"name": "close", "t0": t_desc, "t1": t_hold})

    # -- retreat: straight back along the approach, then straight up to carry height --
    ...
    phases.append({"name": "lift", "t0": t_hold, "t1": t})

    # -- carry: elevated Bezier to above the place standoff (boxes inflated by the held object) --
    ...
    routing["legs"].append({"name": "carry", "inserted": leg.inserted, "clear": leg.clear,
                            "blockers": list(leg.blockers)})
    if not leg.clear:
        raise PlanError(f"carry leg cannot be routed clear (blockers: {leg.blockers})")
    t0_carry = t
    for tt, p, via in curve.leg_waypoints(leg, t, CARRY_SPEED):
        wps.append(_wp(tt, p, via=via, **ori))
        t = tt
    phases.append({"name": "carry", "t0": t0_carry, "t1": t})
    ...
    phases.append({"name": "place", "t0": t, "t1": t_place})

    # -- release + retreat + park --
    release_start = t_place + 0.2
    release_end = release_start + RELEASE_DUR
    # Hold the release pose until the jaw is fully open — rising earlier would drag the object.
    ...
    t_end = t_up + 1.4
    park_end = np.array([parked[0], parked[1], max(parked[2], place_standoff[2])])
    wps.append(_wp(t_end, park_end, **neutral))
    phases.append({"name": "release", "t0": t_place, "t1": t_up})
    phases.append({"name": "park", "t0": t_up, "t1": t_end})
    ...
    if pick.adjusted and pick.adjusted.get("force_target_n") is not None:
        force_target = float(pick.adjusted["force_target_n"])   # LLM retry override (clamped there)
    ...
    return TrajPlan(waypoints=wps, grasp_window=window, num_frames=num_frames,
                    pick=pick_summary, place=place, phases=phases, target=target,
                    target_label=label, target_ordinal=ordinal, routing=routing,
                    attempt=attempt)
