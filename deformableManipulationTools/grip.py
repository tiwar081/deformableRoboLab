...
FINGER_BOX_SLICES = 4
# Uniform slab width as a fraction of the pad half-width. <1 because the rounded pad edges don't reach
# the tip — a full-width slab would be forced very short to stay inside the mesh.
FINGER_WIDTH_FRAC = 0.65
# Depth fraction covered by the single wide FRONT box (the tip-reaching, ~constant-length part of the
# pad); the remaining depth (the taper) is split into the other FINGER_BOX_SLICES-1 boxes.
FINGER_FRONT_FRAC = 0.5


def _finger_collider_points(robot_builder, shape_idx):
    """All finger-collider vertices in the finger BODY frame (a box -> its 8 corners, a mesh -> its
    vertices), stacked (N, 3). The raw geometry _finger_box_slices approximates."""
    pts_all = []
    for s in shape_idx:
        ...
        pts_all.append(quat_rotate_xyzw(q, pts) + off)
...
def _hull_length_range(A, c, length_ax, width_ax, depth_ax, w, d):
    ...
    neg = al < -1.0e-12
    if pos.any():
        ceil = float((rhs[pos] / al[pos]).min())
    if neg.any():
        floor = float((rhs[neg] / al[neg]).max())
    return floor, ceil
...
def _finger_box_slices(points, n_slices):
    ...
    for a, b, floor, ceil in raw:
        ...
        blo = np.zeros(3)
        bhi = np.zeros(3)
        blo[depth_ax], bhi[depth_ax] = a, b
        blo[width_ax], bhi[width_ax] = wc - wr, wc + wr
        blo[length_ax], bhi[length_ax] = top, ceil
        boxes.append((0.5 * (blo + bhi), 0.5 * (bhi - blo)))
    return boxes
...
def build_gripper_proxies(object_builder, robot_builder, finger_bodies: list[int],
    ...
    finger_shape_groups: list[list[int]] = []            # per-finger shapes (to filter left<->right below)
    ...
    for label, finger_body in zip(("left_gripper_contact_proxy", "right_gripper_contact_proxy"),
        ...
        # The finger proxy is the robot's OWN finger colliders, copied one-for-one (the DEFAULT):
        #   * BOX colliders (FR3 URDF: sparse pad/edge/knuckle boxes) → exact copies.
        #   * a CONVEX_MESH collider (Isaac Sim Panda USD: one mesh per finger) → the mesh itself,
        ...
        if (not box_slice_proxy) or all(int(robot_builder.shape_type[s]) == int(newton.GeoType.BOX)
                                        for s in finger_shape_idx):
            for n, s in enumerate(finger_shape_idx):
                # A CONVEX_MESH source is DEEP-COPIED into the object model, never shared with the robot
                # builder: sharing the same Mesh object means both models' BVHs alias one pool, and the
                # viz-first finalize frees it out from under the object narrow phase (the documented
                # shared-mesh-BVH gotcha — CLAUDE.md/SOLVERS §4), which faults GJK-MPR at runtime.
                src = robot_builder.shape_source[s]
                if src is not None and hasattr(src, "vertices"):        # a MESH/CONVEX_MESH source
                    src = src.copy() if hasattr(src, "copy") else newton.Mesh(
                        np.array(src.vertices, dtype=np.float32).copy(),
                        np.array(src.indices, dtype=np.int32).copy())
                shape = object_builder.add_shape(
                    body=body, type=robot_builder.shape_type[s], xform=robot_builder.shape_transform[s],
                    cfg=proxy_cfg, scale=robot_builder.shape_scale[s], src=src,
                    label=f"{label}_shape_{n}")
                proxy_shapes.append(shape)
        else:
            pts = _finger_collider_points(robot_builder, finger_shape_idx)
            for k, (center, half) in enumerate(_finger_box_slices(pts, FINGER_BOX_SLICES)):
                shape = object_builder.add_shape_box(
                    ...
                    label=f"{label}_slice_{k}")
                proxy_shapes.append(shape)
        finger_shape_groups.append(proxy_shapes[group_start:])
    finger_shapes = list(proxy_shapes)
    # Filter the LEFT finger's shapes against the RIGHT finger's: the two jaws grip an object BETWEEN
    # them and must never collide each OTHER. Harmless for box pads (they route through the robust
    # primitive narrow phase), but ESSENTIAL for the default mesh pads: the two convex-mesh fingers
    # would be the only unfiltered mesh<->mesh pair, and Newton's GJK-MPR narrow phase faults
    # (CUDA error 700) on that pair the moment they near the work — filtering removes it.
    if len(finger_shape_groups) == 2:
        for ls in finger_shape_groups[0]:
            for rs in finger_shape_groups[1]:
                object_builder.add_shape_collision_filter_pair(ls, rs)
...
def _grip_force_stop_kernel(
    ...
    grip_rate_max: float,                       # [m/s] jaw rate limit BOTH ways + approach speed (physical)
            ...
            if force_stop_enabled == 0:
                ...
                if engaged < 0.5:
                    ...
                    if err < -grip_force_deadband:
                        w_dot = k_close * (err + grip_force_deadband)     # < 0: close (responsive gain)
                        w_dot = wp.max(w_dot, -grip_rate_max)             # physical rate cap
                    elif err > grip_force_deadband:
                        w_dot = k_open * (err - grip_force_deadband)      # > 0: open (reluctant gain —
                                                                          # the anti-drop asymmetry)
                        w_dot = wp.min(w_dot, grip_rate_max)              # physical rate cap (symmetric)
...
class GripController:
    ...
    (the per-window close/open gains + deadband from ``GripConfig.window_params(force_target)`` —
    the open gain is k_close/k_open_ratio, the anti-drop asymmetry; the fixed ``force_filter_tau`` /
    the symmetric physical ``grip_rate_max`` / engage clamp).
    ...
    def step(self, substep: int, finger_q) -> None:
        wp.launch(_grip_force_stop_kernel, dim=1, inputs=[
            ...
            self.engage_frac, self.engage_floor, self.engage_cap, self.squeeze_signal,
